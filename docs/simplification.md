...
| --- | --- | --- | --- | --- |
| A7 | Students scan every dated deadline with less markup; undated awards remain in the linked directories. | HTML 344,057 to 157,946 bytes (-54.09%); gzip 41,841 to 28,085. All 699 dated rows retain their month, name, amount, status and link without JavaScript. | 0/4 units: row class labels (103,529 B), two span wrapper families (9,087 B each excluding labels), undated duplicate list (66,713 B). | One cut round; corrected an optional-deadline type in the new E2E test on the first repair. 945 unit tests, 83 E2E passes (7 existing skips); ship-check PASS and both RUN commands complete. Metadata and guide bodies unchanged. |
| A6 | Students keep a private offline shortlist with shorter cards; remove duplicated paragraphs, derived listing URLs and repeated field labels. | HTML 526,082 to 244,003 bytes (-53.62%); gzip 96,651 to 46,185. All 1,263 retained card facts/status flags/calendar URLs and derived links match. | 0/4 units: audience (89,129 B), description (38,965 B), derived href (78,740 B), repeated keys (remaining 75 KB saving, net). | One cut round, no failed repairs; inline catalogue preserves offline availability without sending bookmark IDs. 945 unit tests, 81 E2E passes (7 existing skips), including offline reload/export; ship-check PASS and both RUN commands complete. |
| A5 | Students get the same ordered matches with less quiz data; keep full eligibility on listing pages and match reasons beside results. | Payload 832,184 to 402,321 bytes (-51.65%); 4,189 before/after answer sets match exactly, including every question/board/school option and 4,000 seeded full answers. | 0/4 units: default criteria values (318 KB), result audience paragraphs (89 KB), unused program eligibility (14 KB) and location (8 KB). | One cut round; no failed repair. Known empty criteria stay distinct from unknown criteria. 945 unit tests, 79 E2E passes (7 existing skips); ship-check PASS and both RUN commands complete. |
| A4 | Students load listing pages without a router or other pages' styles; keep native Back, private analytics, quiz cleanup and the existing fonts. | 153,432 to 81,567 resource bytes (-46.84% this target); mission baseline 161,616 (-49.53%). | 3/9 units (33.33%): restore quiz departure cleanup, shared quiz-button styles on Saved, and Saved card heights. | Three cutting rounds; post-check inspection caught Saved links shrinking below 44px, now covered by E2E. Stopped 759 bytes above mission half with 54,192 font bytes retained. Native-navigation test observations updated; key layout/fonts/colors match at 375/1440. 943 unit tests, 79 E2E passes (7 existing skips); ship-check PASS and both RUN commands complete. |
| A3 | Students get shorter copy with the same privacy choices, quiz and guide destinations; remove duplicated browse sections and explanatory prose. | Words: home 414 to 202; about 304 to 131; educators 337 to 117; match 490 to 133; guides 740 to 265; updates 3,167 to 1,226; 404 72 to 26. | 0/8 units; seven page-copy blocks and consent copy. | One cut round; all 125 updates retained; metadata and 18 guide bodies identical. 943 unit tests, 79 E2E passes (7 existing skips), ship-check PASS and RUN commands complete; update-log readership question in PR. |

...
import { META_MAX, TITLE_MAX, brandedTitle } from './meta.ts';
...
describe('relatedListings', () => {
  it('names listings that actually exist, in either dataset', () => {
    // Programs are eligible too: a guide whose subject is a research program
    // needs the same reciprocal link, and restricting this to scholarships is
    // what kept /programs/ out of the guide link graph entirely.
    const known = new Set([
      ...(scholarships as { title: string }[]).map(s => generateSlug(s.title)),
      ...(programs as { name: string }[]).map(p => generateSlug(p.name)),
    ]);
    for (const g of guides) {
...
describe('meta descriptions', () => {
  ...
  it('have titles that fit once the brand is added, or drop it', () => {
    // The descriptions have been guarded since 2026-08-19; the titles were not,
    // and a guide title is written by hand with nothing to catch a long one.
    for (const g of guides) {
      const t = brandedTitle(g.title);
      expect(
        t.length <= TITLE_MAX || t === g.title.trim(),
        `${g.slug} renders a ${t.length}-char title`,
      ).toBe(true);
    }
  });

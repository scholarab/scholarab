...
describe('filterSortScholarships', () => {
  ...
  // The chips are derived from the facet registry now, so any city in the data
  // is a legal key. Red Deer is the case that used to be unreachable: it has a
  // hub page and a guide link, and before the chips were derived it had no
  // control anywhere that could select it.
  it('region filters: a city key that is not Medicine Hat matches that city exactly', () => {
    const items = [
      makeScholarship({ id: 1, region: 'Medicine Hat', _deadline_ms: FUTURE_MS }),
      makeScholarship({ id: 2, region: 'Red Deer', _deadline_ms: FUTURE_MS }),
      makeScholarship({ id: 3, region: 'Calgary', _deadline_ms: FUTURE_MS }),
      makeScholarship({ id: 4, region: 'Alberta', _deadline_ms: FUTURE_MS }),
    ]
    expect(ids(filterSortScholarships(items, state({ selectedRegion: 'Red Deer' })))).toEqual([2])
    expect(ids(filterSortScholarships(items, state({ selectedRegion: 'Calgary' })))).toEqual([3])
    // 'Alberta' is a data value, not a chip: the chip is 'Alberta-wide', which
    // rolls the cities in. Asking for the literal must not silently do that.
    expect(ids(filterSortScholarships(items, state({ selectedRegion: 'Alberta' })))).toEqual([4])
  })

  it('every region chip in the facet registry is a key filterSortScholarships understands', () => {
    const items = SCHOLARSHIP_FACETS
      .filter(f => f.kind === 'region')
      .map((f, i) => makeScholarship({ id: i + 1, region: f.value, _deadline_ms: FUTURE_MS }))
    for (const f of SCHOLARSHIP_FACETS.filter(f => f.kind === 'region')) {
      const got = filterSortScholarships(items, state({ selectedRegion: f.value }))
      expect(got.map(s => s.region)).toEqual([f.value])
    }
  })

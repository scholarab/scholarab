...
describe('quiz storage', () => {
  it('pins the storage key both surfaces write', () => {
    // Bumping this is a deliberate migration, not a rename; the quiz and any
    // stored answers in the wild key off this string.
...
describe('quiz hints', () => {
  it('never lets one hint cover most of a question', () => {
    ...
    for (const q of QUIZ_QUESTIONS) {
      ...
      // Options without a hint are not a repeated hint; only real text counts.
      for (const o of q.opts) if (o.hint) counts.set(o.hint, (counts.get(o.hint) ?? 0) + 1);

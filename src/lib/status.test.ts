...
describe('programIsListed', () => {
  const today = new Date('2026-08-22T00:00:00');

  it('counts what /programs actually shows', () => {
    expect(programIsListed({ deadline: '2026-09-01' }, today)).toBe(true);
    expect(programIsListed({ deadline: 'TBA' }, today)).toBe(true);
    expect(programIsListed({ deadline: 'Ongoing' }, today)).toBe(true);
    expect(programIsListed({ deadline: null }, today)).toBe(true);
  });

  it('drops retired programs, which is the whole point', () => {
    // /educators counted these and told counsellors 125 while the directory
    // they were about to open listed 117.
    expect(programIsListed({ deadline: 'TBA', active: false }, today)).toBe(false);
    expect(programIsListed({ deadline: '2026-09-01', active: false }, today)).toBe(false);
  });

  it('drops a cycle whose deadline has passed', () => {

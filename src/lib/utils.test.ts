...
describe('generateSlug', () => {
  ...
  it('folds accented letters to their base letter', () => {
    expect(generateSlug('Café Award')).toBe('cafe-award')
  })

  // The two real ones. Dropping the accent instead of folding it put the
  // Belcourt listing at /scholarships/belcourt-brosseau-mtis-awards/.
  it('folds accents in listing titles that are actually in the corpus', () => {
    expect(generateSlug('Belcourt Brosseau Métis Awards')).toBe('belcourt-brosseau-metis-awards')
    expect(generateSlug('First Nations, Métis and Inuit Bursary')).toBe('first-nations-metis-and-inuit-bursary')
  })

  // Storing the plain-ASCII title was the old workaround, so the accented and
  // unaccented spellings have to agree or restoring an accent moves a live URL.
  it('gives an accented title the same slug as its unaccented spelling', () => {
    expect(generateSlug('Fogolâr Furlan di Calgary Italian Award'))
      .toBe(generateSlug('Fogolar Furlan di Calgary Italian Award'))
    expect(generateSlug('Ben Sauvé Artstrek Bursary'))
      .toBe(generateSlug('Ben Sauve Artstrek Bursary'))
  })

  it('drops characters with no ASCII base rather than transliterating', () => {
    expect(generateSlug('Æther 日本 Award')).toBe('ther-award')
  })
})
...
describe('formatVerifiedMonth', () => {
  ...
  it('returns null rather than inventing a month it was not given', () => {
    expect(formatVerifiedMonth(null)).toBeNull();
    expect(formatVerifiedMonth(undefined)).toBeNull();
    expect(formatVerifiedMonth('')).toBeNull();

...
/**
 * Count this parse against the hourly cap and say whether it may run.
 *
 * Records first, then counts, which is the whole point. As a SELECT followed
 * by an INSERT this was a check-then-act with nothing serialising the halves:
 * concurrent POSTs all read the same pre-insert count and all passed, so the
 * only bound on a burst was how many requests were in flight, not the cap.
 * That is the same race hitRateLimit was written to close, and this one guards
 * a metered API someone pays for.
 *
 * Inserting first cannot admit too many: every caller's own row is committed
 * before it counts, so each sees itself plus everything already committed. A
 * simultaneous burst may over-reject by a request or two, which is the safe
 * direction for a spend limit.
 *
 * `>` not `>=`, because the count now includes the attempt being decided;
 * the same convention, and the same budget, as hitRateLimit.
 */
async function checkAndLogParseRateLimit(): Promise<boolean> {
  ...
  return (rows[0]?.count ?? 1) <= AI_PARSE_LIMIT
}

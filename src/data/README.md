...
| `ab-high-schools.json` | Alberta high schools; reference list for outreach targeting |
| `ab-districts.json` | Alberta school districts and boards |

No code reads `ab-high-schools.json` or `ab-districts.json`; they are a
reference extract from Alberta Education open data, not a build input. The
"for eligibility matching" note that used to sit in this table described an
intention that was never wired up.

**Contact emails were stripped from both files on 2026-08-22.** They held 695
school and district addresses, 144 of them naming an individual. Everything in
this directory is offered under CC BY-SA, which invites commercial reuse, and
that should not mean handing anyone a ready-made mailing list of Alberta
educators. Outreach contacts, with the provenance CASL requires, live in
`outreach/*.csv`, which is gitignored and must stay that way.
